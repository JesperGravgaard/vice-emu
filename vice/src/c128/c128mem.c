...
/* Two parallel sets of base bank tables, selected at access time by the
 * c128_full_banks resource.  The 128-mode tables expose ram00 and ram01;
 * the 256-mode tables additionally expose ram02 and ram03.  Each set
 * has its own mem_bank_dynamic helper instance; get_bank_dyn() returns
 * the active one based on c128_full_banks.
 *
 * NUM_BASE_BANKS_* is the number of valid entries in each table set
 * (the count excludes the trailing NULL/-1 sentinel). */
...
static const char *banknames256[NUM_BASE_BANKS_256 + 1] = {
    "default",
    "cpu",
    "ram",
    "rom",
...
static const int banknums128[NUM_BASE_BANKS_128 + 1] = {
    bank128_cpu, /* default */
    bank128_cpu,
    bank128_ram,
    bank128_rom,
...
static const int banknums256[NUM_BASE_BANKS_256 + 1] = {
    bank256_cpu, /* default */
    bank256_cpu,
    bank256_ram,
    bank256_rom,
...
static const int bankindex128[NUM_BASE_BANKS_128 + 1] = {
    -1,
    -1,
    -1,
    -1,
...
static const int bankindex256[NUM_BASE_BANKS_256 + 1] = {
    -1,
    -1,
    -1,
    -1,
...
static const int bankflags128[NUM_BASE_BANKS_128 + 1] = {
    0,
    0,
    0,
    0,
...
static const int bankflags256[NUM_BASE_BANKS_256 + 1] = {
    0,
    0,
    0,
    0,
...
/* Two configs -- one per mode -- selected at access time by c128_full_banks.
   Each mode owns its own helper instance so the cached arrays remain valid
   across mode switches without an explicit invalidation. */
static const mem_bank_dynamic_config_t bank_dyn_config_128 = {
    .base_banknames   = banknames128,
    .base_banknums    = banknums128,
    .base_bankindex   = bankindex128,
    .base_bankflags   = bankflags128,
    .num_base_banks   = NUM_BASE_BANKS_128,
};
static const mem_bank_dynamic_config_t bank_dyn_config_256 = {
    .base_banknames   = banknames256,
    .base_banknums    = banknums256,
    .base_bankindex   = bankindex256,
    .base_bankflags   = bankflags256,
    .num_base_banks   = NUM_BASE_BANKS_256,
};
static mem_bank_dynamic_t *bank_dyn_128;
static mem_bank_dynamic_t *bank_dyn_256;

static mem_bank_dynamic_t *get_bank_dyn(void)
{
    if (c128_full_banks) {
        if (bank_dyn_256 == NULL) {
            bank_dyn_256 = mem_bank_dynamic_create(&bank_dyn_config_256);
        }
        return bank_dyn_256;
    }
    if (bank_dyn_128 == NULL) {
        bank_dyn_128 = mem_bank_dynamic_create(&bank_dyn_config_128);
    }
    return bank_dyn_128;
}
...
uint8_t mem_bank_read(int bank, uint16_t addr, void *context)
{
    ...
    /* Cartridge banks -- dispatch before 128->256 translation. */
